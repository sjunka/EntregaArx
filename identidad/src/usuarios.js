...
const aUsuario = (f) => f && ({
  ...
  cedula: f.cedula, correoContacto: f.correo_contacto,
})
...
export function crearUsuarios(db) {
  ...
  return {
    ...
    crear: async (u) => (await db.query(
      `INSERT INTO usuarios (cuenta, clave, habilitado, nombres, apellidos, cedula, correo_contacto)
       VALUES ($1, $2, $3, $4, $5, $6, $7) ON CONFLICT (cuenta) DO NOTHING RETURNING id`,
      [u.cuenta, u.clave, u.habilitado, u.nombres, u.apellidos, u.cedula, u.correoContacto])).rows[0]?.id ?? null,
