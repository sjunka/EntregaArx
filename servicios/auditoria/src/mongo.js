...
const aAcceso = (d) => ({
  id: String(d._id), documentoId: d.documentoId, titulo: d.titulo, accion: d.accion, actor: d.actor, ocurridoEn: d.ocurridoEn.toISOString(),
})
...
export function crearRepo(db) {
  ...
  return {
    ...
    async agregar({ eventoId, documentoId, cedula, titulo, accion, actor, ocurridoEn }) {
      try {
        await col.insertOne({ eventoId, documentoId, cedula, titulo, accion, actor, ocurridoEn: new Date(ocurridoEn), registradoEn: new Date() })
      } catch (e) {
