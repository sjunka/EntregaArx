...
export function crearApp({ emisor, llave, usuarios, clientes, administradores = {}, servicios = {}, origenes = [] }) {
  ...
  app.post(`${oidc}/token`, async (req, res) => {
    ...
    const perfil = {
      ...
      ...(u.analista && { analista: true }), // HU-12: el analista del Estado no tiene carpeta: sin cédula
    }
    res.set('cache-control', 'no-store').json({
      ...
      access_token: await firmar({ ...perfil, typ: 'Bearer', scope: 'openid profile email', ...(!u.empresa && !u.analista && { cedula: u.cedula }) }, u.empresa ? 'premium' : u.analista ? 'analitica' : AUDIENCIAS_CIUDADANO, VIDA_TOKEN),
